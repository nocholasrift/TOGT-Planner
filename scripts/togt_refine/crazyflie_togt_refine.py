...
def refine(quad, planner, tol, tol_term, desired_dt):
    q = Quadrotor(ROOTPATH+'/parameters/'+quad+'/'+quad+'_quad.yaml')
    togt = Trajectory(desired_dt, ROOTPATH+"/resources/trajectory/"+quad+"_traj.csv", ROOTPATH+"/resources/trajectory/"+quad+"_wpt.yaml")
    togt.print()

    wp_opt = Optimization(q, togt._wpt_num, togt._Ns, tol, tol_term)
    ...
    save_traj(res_t, wp_opt, ROOTPATH+"/resources/trajectory/"+quad+"_refined_traj.csv")
